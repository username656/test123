...
def api_request(module, url, method="GET", headers=None, data=None):
    service_token = module.params.get('service_token')
    body = None
    if data:
    ...
    if int(info['status']) == -1:
        module.fail_json(msg="Failed to execute the API request: %s" % info, url=url, method=method, headers=headers)
...
def k8s_delete_resource(module, url, data):
    ...
    if info['status'] == 404:
        ...
        module.fail_json(msg="failed to delete the resource '%s': %s" % (name, info), url=url)
...
def k8s_replace_resource(module, url, data):
    ...
    if info['status'] == 409:
        ...
        module.fail_json(msg="failed to replace the resource '%s': %s" % (name, info), url=url)
    return True, body
...
def k8s_update_resource(module, url, data, patch_operation):
    ...
    if info['status'] == 409:
        ...
        module.fail_json(msg="failed to update the resource '%s': %s" % (name, info), url=url)
    return True, body
